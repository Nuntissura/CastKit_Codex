# Technical Specification — CastKit Codex (CKC) — v00.039
...
- Protected fields:
  ...
  - `CHAR-ID-001` is treated as system-managed: it is enforced to match the internal `characterId` on save/import and is surfaced in UI as a copyable ID (not a normal editable field).
...
- Provide an explicit “Import from URL” action (Library/Inbox and Character) that downloads a file and imports it through the normal ingest pipeline.
- Store per-asset provenance metadata:
  - `source_url`
  - Optional `source_note` (free text)
...
- Saved searches support rule-based definitions (not just an ad-hoc current filter snapshot), including:
  - Tags include/exclude (AND/OR)
  - Rating operator/value
  - Favorite only
  - Scope (global vs current character)
- UI supports editing rules and previewing result counts.

### 11.13 Color tools: palettes + search (WP-0066)
- Compute and store an image’s dominant palette (small set of colors) for fast browsing.
- Provide a “filter by color” control (pick a color chip; show similar items).
- Color extraction is lazy and cached (no rework every time).
