# Technical Specification — CastKit Codex (CKC) — v00.074
...
## v00.074 - WP-0115 multi-rig workspace tabs (2026-05-07)

This version adds session-scoped multi-rig workspace tabs to the Pose tab. Durable pose data remains in the existing `Rig` rows and image assets; open-tab order and active-tab selection live only in the running backend session.

### Workspace State
- Backend session state tracks ordered open rig IDs, the active rig ID, and optional transient state per rig.
- New backend/IPC/preload/automation commands: `listOpenRigs`, `openRigWorkspace`, `setActiveRig`, `closeRigWorkspace`, and `reorderOpenRigWorkspaces`.
- Closing a workspace removes only the session tab. It does not delete the `Rig` row, portrait image, openpose exports, workflow history, or identity profiles.

### Pose UI
- The Pose tab now renders an accessible workspace tab strip above the rig workspace with `tablist` / `tab` / `tabpanel` semantics, active-tab `aria-selected`, close buttons, and stable automation selectors.
- Keyboard support covers Left/Right, Home/End, and Delete-to-close on workspace tabs.
- Selecting a rig from the Library panel opens/activates that rig workspace. Creating or detecting a rig also opens the resulting rig as the active workspace.
- Switching or closing an active tab persists dirty calibration before changing the active rig. A visible dirty indicator marks unsaved calibration on the active workspace.

### Research Applied
- WAI-ARIA APG and MDN tab guidance drove the role/keyboard implementation, including optional Delete close behavior and coordinated `aria-selected` / `aria-controls`.
- React's current state-identity guidance pushed the implementation toward stable rig IDs and lifted session state rather than index-keyed component identity.
- Electron's current contextBridge guidance kept the IPC surface narrow: renderer code receives typed preload methods instead of direct main-process or Node access.
- Hugging Face/Civitai/practitioner scans did not reveal a more relevant rig-workspace pattern than accessible tabs plus compact in-app reference switching; drag/reorder UI is deferred until CKC has an established drag pattern, while backend reorder is already implemented and tested.

### Tests and Live Gate
- `CKC_main/test/rig_workspace_tabs.test.js`
- `CKC_main/test/posekit_ui_static.test.js`
- `CKC_main/test/automation_manual_consistency.test.js`
- `npx tsc --noEmit`
- Live gate used sample images copied from `D:/Projects/LLM projects/OpenRepose/test_material/image_samples` into `CKC_GOV/references/posekit_samples/`. Character `char_c2fcc4dcd3f6316682239f968df787ed` opened rigs `rig_69deb074d2fc64941d0f954325704b07` and `rig_4b3063cf3132a41077cc87ed907eec9d`; the tab strip rendered two tabs, per-tab Tools/Markers state restored, active rig switched from yaw/pitch/roll `35/5/-5` to `0/0/0` after closing the active tab, and both Rig rows remained in the backend. Captures: `2026-05-07_200417170Z_no_session_wp-0115-final-two-rig-tabs.png` and `2026-05-07_200417510Z_no_session_wp-0115-final-after-close.png`.

---

## v00.073 - WP-0113 hand detection + openpose hand keypoints (2026-05-07)

This version extends PoseKit detection from body-18 + face-70 to body-18 + face-70 + hand-21. The output remains canonical OpenPose JSON: `hand_left_keypoints_2d` and `hand_right_keypoints_2d` are always 63 floats each, zero-filled when no hand passes confidence gating and populated when MediaPipe HandLandmarker detects a hand.

### Hand Detection
- Bundled model asset: `CKC_main/public/posekit_models/hand_landmarker.task` from the official MediaPipe HandLandmarker float16 model path.
- The existing PoseKit worker now creates `HandLandmarker` with `runningMode: "IMAGE"`, `numHands: 2`, and 0.5 confidence thresholds, alongside the existing pose and face tasks.
- `HAND_21` is the canonical taxonomy: wrist, thumb CMC/MCP/IP/tip, index/middle/ring/pinky MCP/PIP/DIP/tip. MediaPipe and OpenPose hand order align directly, so CKC does not remap indices.
- Handedness maps detected hands into `RigData.handLeft` and `RigData.handRight`. Duplicate side detections keep the highest handedness score. CKC uses handedness/presence confidence rather than `visibility`, because MediaPipe JS image-mode can return valid hand landmarks with `visibility: 0`.

### Rendering and Calibration
- `rigToOpenposeJson`, yaw/head-pose transforms, calibration transforms, and 2D canvas rendering already handled hand arrays; WP-0113 fills those arrays from detection and switches hand skeleton line color to cyan.
- `Pose3DViewport` now renders hand spheres and bones for both hands.
- Pose tab shows `hands 0/2`, `1/2`, or `2/2` in the stage overlay and preview data. Tools / Markers exposes left/right hand visibility toggles that update per-keypoint calibration visibility for all 21 points on that side.

### Tests and Live Gate
- `CKC_main/test/hand_detection_taxonomy.test.js`
- `CKC_main/test/hand_openpose_export.test.js`
- `CKC_main/test/posekit_core.test.js`
- `CKC_main/test/pose_head_pose_math.test.js`
- `CKC_main/test/posekit_ui_static.test.js`
- `CKC_main/test/automation_manual_consistency.test.js`
- `npx tsc --noEmit`
- `npm run build`
- Live gate against the operator sample folder confirmed the copied WP-0113/WP-0111/WP-0115 samples run through `mediapipe.tasks-vision.pose+face+hand` and emit zero-filled 63-float hand arrays when no hand passes gating. Hand-positive live gate used `CKC_GOV/references/posekit_samples/wp-0113-mediapipe-hand-demo.jpg`, producing rig `rig_26e1e52789758372e0d0e88618b9b9dd`, `handRight.length=21`, nonzero `hand_right_keypoints_2d`, and captures `2026-05-07_193516138Z_no_session_wp-0113-hand-positive-live.png`, `2026-05-07_193516766Z_no_session_wp-0113-hand-positive-3d.png`, and `2026-05-07_193517389Z_no_session_wp-0113-hand-positive-toggles.png`.

---

## v00.072 - WP-0111 identity export profiles (2026-05-07)

This version adds per-character identity profiles for PoseKit and ComfyUI replay. A profile stores a locked face reference derived from a rigged portrait: a 512x512 content-hash-addressed face PNG, face landmarks, deterministic measurements, pose-at-capture metadata, and a bridge payload that downstream workflows can consume as an IPAdapter / face-reference input.

### Identity Profile Storage
- New additive table: `IdentityProfile`, keyed by `profile_id`, linked to `Character`, source `ImageAsset`, source `Rig`, and cropped-face `ImageAsset`.
- Per-character folders now include `images/identity/` for content-hash-addressed profile PNGs and `identity_profiles/` for JSON sidecar manifests.
- Profile rows persist `face_landmarks_json`, `feature_measurements_json`, `pose_metadata_json`, `bridge_payload_json`, `manifest_relative_path`, timestamps, and `deleted_at` for soft-delete.
- The crop path is deterministic for the same source image + rig landmarks. Electron uses `nativeImage` crop/resize when available; a pure-JS deterministic PNG fallback keeps backend tests stable outside Electron.

### Backend, UI, and Workflow Replay
- Backend/IPC/preload/automation now expose `listIdentityProfiles`, `getIdentityProfile`, `createIdentityProfile`, `updateIdentityProfile`, and `deleteIdentityProfile`.
- Pose tab adds an Identity panel with create/list/delete controls and profile thumbnails for the active character.
- Workflow replay adds an Identity profile dropdown. `replayWorkflow({ identityProfileId })` validates ownership and injects identity fields into `CastKitCodexBridge` inputs when those optional sockets exist.
- The CKC ComfyUI bridge node now accepts optional identity fields: `identity_profile_id`, `identity_profile_ref`, `identity_image_id`, and `ipadapter_image_ref`, with matching environment fallbacks for external bridge use.

### Research Applied
- Current IPAdapter FaceID, InstantID, PuLID, and ReActor practice expects stable face-reference images and/or 5-point/ArcFace-style identity metadata, with node-specific embedding extraction happening in downstream ComfyUI packs.
- CKC ships the portable core profile contract now: crop, landmarks, measurements, pose metadata, and bridge references. Heavyweight InsightFace / ArcFace / EVA-CLIP embedding caches remain a future extension so WP-0111 does not add model runtimes or vendor-specific binary dependencies to CKC.
- The bridge payload is intentionally a single CKC profile reference plus cropped image reference; custom ComfyUI workflows can split it into IPAdapter, InstantID, PuLID, or face-swap sockets as needed.

### Tests and Live Gate
- `CKC_main/test/identity_profile_crud.test.js`
- `CKC_main/test/identity_profile_replay_injection.test.js`
- `CKC_main/test/backend_posekit_workflow.test.js`
- `CKC_main/test/posekit_ui_static.test.js`
- `CKC_main/test/automation_manual_consistency.test.js`
- `npx tsc --noEmit`
- Live smoke used `D:/Projects/LLM projects/OpenRepose/test_material/image_samples/1085406391.jpg`, created character `CHAR-000005`, detected MediaPipe rig `rig_241ea5569a419fdd4395eff89d70386e`, created profile `idp_562fb122ff50282c7f697a30ae362fc2`, wrote crop `images/identity/d7f57768109c90a0.png`, and captured `CKC_GOV/targets/CKC/automation_captures/2026-05-07_185915473Z_no_session_wp-0111-identity-panel.png` plus `2026-05-07_185916851Z_no_session_wp-0111-workflow-identity-dropdown.png`.

---

## v00.071 - WP-0110 pitch/roll head pose extension (2026-05-07)

This version extends PoseKit from yaw-only head rotation to full yaw + pitch + roll head pose. Head pose is stored as an intrinsic `YXZ` quaternion plus explicit angle metadata so UI controls stay readable while math/export paths avoid Euler-order ambiguity.

### PoseKit Head Pose
- `Rig.calibration_json.headPose` is the canonical persisted field: `{ schemaVersion: 1, order: "YXZ", yaw, pitch, roll, quaternion: [x, y, z, w] }`.
- Existing rigs without `headPose` still load through the legacy `calibration.yaw` fallback. `calibration.yaw` is also kept in sync with `headPose.yaw` for backward compatibility with WP-0108 callers.
- `createHeadPose`, `normalizeHeadPose`, `applyHeadRotation`, and `applyHeadPose` are the canonical pose-math entry points. `rigToOpenposeJson`, `openposeJsonText`, and `renderRigToCanvas` apply `headPose` before calibration and openpose serialization.
- The 3D Pose viewport applies the same quaternion-backed head pose to the rig, and the 2D openpose canvas renders the projected keypoints at the current yaw/pitch/roll.

### UI and Automation
- The Pose toolbar now exposes yaw, pitch, and roll numeric inputs plus sliders and reset buttons. Ranges: yaw `[-90, 90]`, pitch `[-75, 75]`, roll `[-45, 45]`.
- The Tools / Calibration panel shows the pinned order and quaternion readout.
- Backend/IPC/preload/automation/manual surfaces now include `setRigHeadPose({ rigId, headPose })`. The backend derives a normalized quaternion from angles when callers omit `headPose.quaternion`.

### Research Applied
- OpenPose, DWPose, and ComfyUI controlnet preprocessors do not carry explicit head Euler fields; head pose must be encoded by the projected nose/eye/ear/face keypoint geometry.
- MediaPipe Pose Landmarker does not output head yaw/pitch/roll directly; CKC keeps user-edited PoseKit rig head pose as the source of truth.
- Three.js `Quaternion`/`Euler` support intrinsic `YXZ` composition and are already in the renderer/backend dependency graph; no new math dependency was added.
- 6DRepNet and recent head-pose/identity-drift papers support storing rotation as quaternion/matrix rather than persisting Euler triples as the sole math representation.

### Tests and Live Gate
- `CKC_main/test/pose_head_pose_math.test.js`
- `CKC_main/test/posekit_core.test.js`
- `CKC_main/test/backend_posekit_crud.test.js`
- `CKC_main/test/posekit_ui_static.test.js`
- `CKC_main/test/automation_manual_consistency.test.js`
- `npx tsc --noEmit`, `npm run build`, and `npm test -- --test-reporter=spec` pass (197 pass, 1 skipped).
...
## v00.070 - WP-0106 forward/backward compatibility hardening (2026-05-07)

This version turns the schema and ingestion compatibility law into executable product checks. The guarantee is that data captured under today's v00.19 ingestion contract and current template/schema remains readable, searchable, exportable, and recoverable after later WPs.

### Compatibility Invariants
- Frozen legacy fixtures live under `CKC_main/test/fixtures/legacy/` for `wp-0091`, `wp-0100`, `wp-0103`, and `wp-0104`. Each fixture carries a small SQLite DB, image bytes, sheet file, and `manifest.json`; the current `CKCLibrary` must migrate and load each one.
- Schema migration linting rejects non-additive changes: `ADD COLUMN ... NOT NULL` without `DEFAULT`, unapproved `DROP COLUMN`, unapproved `DROP TABLE`, sacred provenance column renames, and non-idempotent heavy-table index creation.
- Workflow spec versions are pinned to handler files through `CKC_main/app/backend/imageSourcingHandlers/_pinned.json`; v00.19 stays routed to `v00_19.js`.
- Template field IDs for `CHARACTER_SHEET__v2.00.txt` are append-only. The baseline is `CKC_main/test/fixtures/template_v2_00_field_ids.json` with a changelog beside it.
- Re-running `ingestImageSourcingTask` on the same accepted v00.19 task performs no net DB write after dedup; duplicate-only reruns return skipped rows without creating a new `IngestionBatch`.
- Backups now write `ckc_app_version`, `schema_migration_cursor`, `created_at`, `db_provider`, `image_count`, and `character_count` into `manifest.json`. Validation/restore refuses backups made by a newer CKC version.
- ImageAsset index pins include `(character_id, file_hash)`, global `(file_hash)`, `(sheet_version_id)`, `(source_dataset_id, source_task_id)`, and `(review_status, character_id)`.

### Research Applied
- PostgreSQL `CREATE INDEX CONCURRENTLY` documentation confirms concurrent index builds avoid write-blocking locks on hot tables, with caveats.
- SQLite `ALTER TABLE ADD COLUMN` documentation confirms `NOT NULL` additions require a non-null default and constraint checks can scan existing content.
- Prisma's expand/contract migration guidance, AWS live-migration guidance, Google Database Migration Service schema-conversion docs, Hugging Face dataset metadata/version surfaces, Civitai model-version/hash API docs, and X API consistency docs all point toward explicit versioned contracts, additive evolution, stable ids, and local validation before cutover.

### Tests
- `CKC_main/test/legacy_fixture_compatibility.test.js`
- `CKC_main/test/migration_invariants.test.js`
- `CKC_main/test/ingestion_handler_routing.test.js`
- `CKC_main/test/template_field_id_immutability.test.js`
- `CKC_main/test/ingestion_idempotency.test.js`
- `CKC_main/test/backup_version_traceability.test.js`
- `CKC_main/test/spec_canon_consistency.test.js`
- `CKC_main/test/db_index_invariants.test.js`

---

## v00.069 - WP-0105 tiered reinstall and reset modes (2026-05-07)

This version records the four CKC reset modes and the orphan-image recovery contract.

### Reset Modes
- Update and Reinstall preserve app configuration, renderer storage, library files, database rows, and image bytes.
- Light reset deletes `ckc-config.json` plus Electron preference/storage paths (`Preferences`, `Local Storage`, `Session Storage`, `IndexedDB`) without touching CKC content rows or library files.
- Full reset writes `.ckc-pending-full-reset`, then on next startup writes `libraryRoot/orphans/<timestamp>/manifest.json`, removes generated folders (`exports`, `templates`, and per-character `sheet`, `extras`, `packs`, `scripts`), truncates CKC content tables, and preserves `CkcMeta` / `CkcDbMigration`.
- Full reset never deletes files under `characters/<id>/images/original` or `characters/<id>/images/thumb`.

### Orphan Manifest
- Manifest schema is `manifest_version: 1`, `kind: ckc_orphan_images`, `created_at`, `library_root`, `preserved_dirs`, and `entries[]`.
- Each entry records the old `image_id`, `character_id`, `display_name`, `relative_path`, `file_hash`, dimensions, tags, rating, favorite, notes, storage mode, source URL/path/note, WP-0100 provenance fields, sheet version, and review status.
- Backend command `adoptOrphanImages({ manifestPath, targetCharacterId, imageIds })` re-hashes image bytes, skips mismatches, dedupes by `(targetCharacterId, file_hash)`, restores metadata, and creates placeholder characters when `targetCharacterId === "__new__"`.

### Installer and App Hooks
- NSIS uses `scripts/installer_custom.nsh` through `build.nsis.include` and the staged Windows package config.
- App startup checks `.ckc-pending-full-reset` before renderer use, preserves the marker's library root / database provider, retries preference cleanup before files are locked, runs the manifest/truncate path, then reinitializes the library.
- In-app and automation commands: `resetPreferences`, `requestFullReset`, `listOrphanManifests`, `adoptOrphanImages`.

### Tests
- `CKC_main/test/installer_modes_invariants.test.js`
- `CKC_main/test/full_reset_marker.test.js`
- `CKC_main/test/adopt_orphan_images.test.js`
...
- **v00.071 (2026-05-07):** WP-0110 extends PoseKit head pose from yaw-only to yaw + pitch + roll with intrinsic `YXZ` quaternion storage, three-axis UI controls, backend `setRigHeadPose`, and live openpose export verification.
