# Technical Specification — CastKit Codex (CKC) — v00.061
...
- Notes/Stories/Moodboard persistence: DB-first is the source of truth; SQLite remains the default local provider, and PostgreSQL is the target provider for parallel model/operator work.
...
- `database` object (stored in `ckc-config.json`):
  - `provider`: `sqlite` (default) or `postgres`.
  - `connectionString` (PostgreSQL only): PostgreSQL URL.
- Environment overrides:
  - `CKC_DB_PROVIDER=postgres`
  - `CKC_POSTGRES_URL=postgres://user:password@host:5432/database`
- SQLite remains the compatibility default and stores the app DB at `<libraryRoot>/db/codex.db`.
- PostgreSQL is initialized through the DB provider boundary and keeps the current renderer/IPC contracts unchanged.
- SQLite-to-PostgreSQL import is one-way and uses `CKC_main/scripts/sqlite_to_postgres.js` or `CKC_GOV/scripts/sqlite_to_postgres.ps1`.
...
Under `<libraryRoot>`:
- `db/codex.db` — SQLite database when `database.provider` is `sqlite`
