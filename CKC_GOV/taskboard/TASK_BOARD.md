...
| WP-0100 | Image-sourcing workflow spec registry + v00.19 ingestion adapter + per-character scripts + cross-batch dedup | IN_PROGRESS | Codex | Workflow spec registry (read-only) at `CKC_GOV/references/external_app_data/specs/`, multi-version ingestion adapter dispatching by `spec_version` (only `v00_19` handler ships), accepted/pending/rejected lane support (pending → WP-0094 intake sorter, rejected → audit-only), 5 new ImageAsset provenance columns + sheet_version_id (required), CharacterScript table + 4 commands, IngestionBatch + IngestionRejection audit tables, identity-decoupling enforced + tested, 12 new automation commands. Pulls scripts from v00.19-canonical `task_tools/scripts/`. js-yaml dep approved. Will ship as v0.2.9. |

## Current focus
- Current: WP-0100 in progress (planning checkpoint pushed). Image sourcing is ongoing externally for several days; CKC adapter must be ready when accepted/pending images start landing.
...
- Deferred: packaged-build smoke for WP-0099 automation surface; NAS mirror backup.
