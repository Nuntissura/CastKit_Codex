...
| WP-0104 | Block-list inline editor for sheet fields | PLANNED | Codex | Replace the single-textarea fallback for `<list of XYZ_Block \| optional>` fields with a proper inline editor: each block instance renders as a sub-form keyed by the block schema, with add / remove / move-up / move-down. Drops the ~417 empty `ckc-field-*` schema-descriptor divs at the top of every sheet. Per-block sub-field validation reuses the WP-0103 type-aware validator. Spec v00.067 → v00.068. Will ship as packaged build per ship-as-packaged-build memory. |

## Current focus
- Current: v0.2.9 shipped (WP-0099 + WP-0100 packaged build). Live CDP-driven inspection of the running app verified every WP-0099 + WP-0100 item end-to-end (boot, Help drawer, automation surface, workflow spec registry, per-character scripts, ingestion adapter dry-run, identity-decoupling, stealth contract, single-instance lock). First real production character created during the pass: **Aeri / CHAR-000003** with 10 face reference imports tagged by angle, master_base set as character icon (favorite + 5★).
- Bug fixes shipped during the inspection pass (commit `2846ddd`):
  - `db.js initSchema()` returned early for Postgres → `ensureSchemaUpgrades` never ran → all post-WP-0092 column/table additions (incl. WP-0100 `CharacterScript`/`IngestionBatch`/`IngestionRejection`) were missing in production. Fixed.
  - `lib.getDiagnostics` was called by `automationGetState` but never implemented (WP-0093 era). Lightweight implementation added.
- Open minor findings (not yet WPs):
  - Sheet validator misclassifies enum-with-`other:<descriptor>` unions as descriptor-only and rejects valid single-word enum values like `curvy` / `graceful` with "must be 2-12 words" (saves anyway with `allowSaveWithErrors: true`).
  ...
  - `getCharacter().images[].id` (not `imageId`) — manual could spell out the image-object field naming for LLMs.
- Next: WP-0101 legacy flat-folder migrator (karina_blonde-style batches) once in-flight v00.19 batches settle. WP-0102 autonomous LLM workflow runner that drives spawn → sourcing → CKC ingest end-to-end. Possible WP-0103 for sheet-validator descriptor/enum classification fix + sheet-input test suite scaffolding.
- Deferred: packaged-build smoke for WP-0099 + WP-0100 automation surface (run when operator permits); NAS mirror backup; full DB-backed test suite verification.
