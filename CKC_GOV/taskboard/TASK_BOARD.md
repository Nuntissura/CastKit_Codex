...
| WP-0106 | Forward / backward compatibility hardening (frozen-fixture suite + additive-migration linter + handler routing pin + template-field-id immutability + idempotency + backup version traceability + spec-canon consistency + index pinning) | PLANNED | Codex | Locks the schema/ingestion compat invariants from the new "Schema and ingestion forward/backward-compatibility" codex law with executable tests. Frozen legacy fixtures under `CKC_main/test/fixtures/legacy/` (one per major schema state); migration linter rejects NOT-NULL-without-default, DROP COLUMN without DEPRECATIONS, DROP TABLE, renames of pinned-provenance columns, non-CONCURRENT indexes on heavy tables; multi-version handler routing pinned via append-only `_pinned.json`; template field IDs immutable; ingestion idempotency; backup carries `ckc_app_version` + `schema_migration_cursor`; init_task + ingestion adapter cross-validated end-to-end per spec_version. Goal: the operator's stated guarantee — "a 75k-image collection imported under today's contract must still open, search, export, and re-attach after every future WP." Spec bumped (v00.069 → v00.070 if WP-0105 ships first; v00.068 → v00.069 otherwise). Will ship as packaged build per ship-as-packaged-build memory. |
...
- WP-0106 planned: forward/backward-compat hardening. Locks the new "Schema and ingestion forward/backward-compatibility" codex law with frozen legacy fixtures, an additive-migration linter, handler routing pin, template-field-id immutability, ingestion idempotency, backup version traceability, init_task/adapter cross-validation, and index pinning. Pins the operator's guarantee that the ~75k v00.19 images already collected stay usable forever.
- New codex laws (2026-05-06): "Image-sourcing init_task + spec JSON are CKC-governed canon" and "Schema and ingestion forward/backward-compatibility" — both binding on every relevant WP.
