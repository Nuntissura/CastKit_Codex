...
| WP-0103 | Sheet validator + clickElement React-19 + SQL alias regression guard | IN_PROGRESS | Codex | Fix three open bug classes from the post-WP-0100 sheet test campaign: parser misclassifies `<string | unset>` / `<a | b | other:<descriptor>>` / `<score_10 | optional>` unions; validator emits noisy "Non-canonical enum value" on string fields; CDP `clickElement` doesn't reliably trigger React 19 onClick; pin the camelCase SQL alias bug class with a static-grep regression test. Spec bumps v00.066 → v00.067. Ship as v0.2.10. |
