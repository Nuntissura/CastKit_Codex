...
- F1.7. **Block list** (`<list of XYZ_Block | optional>`): rendered as a `<textarea>` with the placeholder `<list of XYZ_Block | optional>`. The operator must type structured content matching the block schema. **GAP (2026-05-06):** no inline block-list editor; the schema descriptor lines (`HUS-BLK-001 — Hustle_Name: <string>` etc.) render as empty `<div id="ckc-field-*">` containers with no input control. Roughly 417 of 896 `ckc-field-*` divs on a fresh sheet are these no-input schema descriptors.
...
- F2.2 UI Save click via CDP — **partial: clickElement now dispatches the full event sequence (WP-0103) but a React-19 specific gap remains.** Even native CDP `Input.dispatchMouseEvent` (trusted events) and direct `__reactProps.onClick` invocation do not complete the CharacterView Save button's `saveSheet` flow. Investigation pending — could be an Electron-renderer / React 19 Concurrent Mode interaction. **Canonical agent test path: backend `window.ckc.saveCharacter`** — documented in the test suite and the in-app manual. Other React-19 buttons (Help drawer, menu) work fine with the new clickElement.
