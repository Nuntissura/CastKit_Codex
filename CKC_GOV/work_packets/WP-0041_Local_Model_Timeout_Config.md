...
## Acceptance criteria
- [x] Local model requests don’t time out at ~60s by default (default timeout increased).
- [x] User can set a longer timeout in the UI and it persists.
- [x] Misconfigured timeout values are clamped to a safe range (no crashes).
